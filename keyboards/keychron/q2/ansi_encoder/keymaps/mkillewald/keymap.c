...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    [MAC_BASE] = LAYOUT_ansi_67(
        ...
        KC_LCTL, KC_LALT, KC_LGUI,                           KC_SPC,                               _______, MO(_FN1), MO(_FN3), KC_LEFT, KC_DOWN, KC_RGHT),
    ...
    switch (keycode) {
        case KC_CHGC1:
            if (record->event.pressed) {
                user_config_set_data(0); // set user data to 0 and save to eeprom
            }
...
void rgb_matrix_set_key_colors(void) {
    ...
    for (uint8_t i = 0; i < sizeof(toprow)/sizeof(toprow[0]); i++) {
        rgb_matrix_set_color(toprow[i], 0, user_config_get_data(), 0);
    }
}